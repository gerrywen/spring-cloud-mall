...
public class OmsOrderServiceImpl implements OmsOrderService {
    ...
    public Result generateOrder(OrderParamDTO orderParam) {
        ...
        if (!hasStock(cartPromotionItemList)) {
            return Result.error(CodeMsg.ORDER_STOCK_NOT_ENOUGH);
        }
        //判断使用使用了优惠券
        if (orderParam.getCouponId() == null) {
            ...
            //使用优惠券
            SmsCouponHistoryDetailVO couponHistoryDetail = getUseCoupon(cartPromotionItemList, orderParam.getCouponId());
            if (couponHistoryDetail == null) {
                return Result.error(CodeMsg.ORDER_COUPON_NOT_USE);
            }
            //对下单商品的优惠券进行处理
        ...
        if (orderParam.getUseIntegration() == null) {
            ...
            if (integrationAmount.compareTo(new BigDecimal(0)) == 0) {
                return Result.error(CodeMsg.ORDER_INTEGRAL_NOT_USE);
            } else {
                //可用情况下分摊到可用商品中
                for (OmsOrderItem orderItem : orderItemList) {
    ...
    private String generateOrderSn(OmsOrder order) {
        ...
        String key = RedisKey.ORDER_KEY_PREFIX_ID + date;
        Long increment = redisUtils.incr(CtimsModelEnum.CTIMS_ORDER_CAP,key, 1);
        sb.append(date);
     ...
     */
    private List<OmsOrderItem> getCouponOrderItemByRelation(SmsCouponHistoryDetailVO couponHistoryDetail, List<OmsOrderItem> orderItemList, int type) {
